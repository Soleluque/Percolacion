...
{ float p=0,sp=0;
  ...
  percolante_n= malloc(sizeof("percolante128.txt"));
...
for(i=0;i<6;i++)
  { p=0;
    sprintf(percolante_n,"percolante%d",*(dims+i));
    percolante= fopen(percolante_n,"w");
    ...
    while(p<=1)
    {
      ...
      for(sem=0;sem<semm;sem++)
      {
        ...
        sp/=(dim*dim); //intensidad del cluster percolante
